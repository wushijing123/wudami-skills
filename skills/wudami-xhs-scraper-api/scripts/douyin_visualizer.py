...
def generate_visual_html(md_path, json_path, output_path):
    ...
    def build_gallery():
        ...
        for i, n in enumerate(notes[:8]):
            ...
            # 如果有视频链接，整张卡片可点击跳转
            card_open  = f'<a class="gallery-card-link" href="{note_url}" target="_blank" title="点击查看原视频">' if note_url else '<div class="gallery-card">'
    ...
    def build_donut_chart():
        ...
        # 尝试匹配各种写法，例如：**维度名称**（占 25%） 或 **维度名**：N 篇（占比 X%）
        dim_pattern = re.compile(r'\*\*(.+?)\*\*(?:[：:]\s*\d+\s*篇|)[（(](?:占|占比)\s*(\d+)%.*?[）)]')
        dimensions = []
        in_section7 = False
        for line in lines:
            if ('维度' in line and '归类' in line) or '选题维度' in line:
                in_section7 = True
                continue
        ...
        gradient_stops = []
        chart_legend = ""
        colors = ["#e9ecef", "#fe2c55", "#ff6b81", "#ff9eaa", "#ffc2c9", "#ffe0e4"]
    ...
    for line_idx, line in enumerate(lines):
        ...
        if line.startswith('# '):
            ...
            if '维度' in current_h2 and ('归类' in current_h2 or '分布' in current_h2 or '选题维度' in current_h2):
                ...
                if best_note:
                    ...
                    if line.startswith('**《') or line.startswith('- **《') or line.startswith('**'):
                        if best_note['title'] not in processed_titles:
                            ...
                            cover_html = f'<a href="{note_url}" target="_blank" title="点击查看原视频"><img src="{cover}" class="side-cover clickable-cover"></a>' if note_url else f'<img src="{cover}" class="side-cover">'
                    ...
                    if note_url:
                        repl_str = f'<a class="tooltip-container" href="{note_url}" target="_blank" title="点击查看原视频">{full_book_mark}<img src="{cover}" class="tooltip-img"></a>'
                    else:
            ...
            if inject_html:
                ...
                # Check for * to support ul properly
                if 'SOP' in current_h2:
                    ...
                    clean_li = styled_line[2:]
                    # Also replace leading * inside if it somehow slipped
                    if clean_li.startswith('* '):
                        clean_li = clean_li[2:]
                    html_output += f'<li>{clean_li}</li>\n'
            ...
            if '词频' in line or '词汇' in line or '词云分析' in line or '5维' in line or '五维' in line:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="生成抖音账号拆解 HTML 报告")
