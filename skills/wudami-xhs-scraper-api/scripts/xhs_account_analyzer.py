...
def analyze_account(data: dict) -> dict:
    ...
    # 不再做关键词维度聚类，改为输出全量笔记标题清单供 AI 做语义维度归类
...
def generate_report(data: dict) -> str:
    ...
    # 全量笔记标题清单（供 AI 做语义维度归类）
    lines.append("## 七、全量笔记清单（供语义维度归类）")
    lines.append('> ⚠️ **AI 核心约束**：Python 不再预设维度名称。请阅读以下全部标题，自行归纳 4-6 个该账号特有的内容维度，每条笔记只归入一个维度。维度名必须具体到该账号内容，禁止使用万能模板标签。')
    lines.append("")
    ...
    lines.append('2. **降维分析声明**：由于小红书反爬拦截缺失了评论数和收藏数，在账号速览中必须公开声明本次爆款评判唯一标准为绝对点赞量。')
    lines.append('3. **剪辑视角排雷**：仅获取了标题和封面图片，未获取视频正文内容，禁止臆想镜头语言/配乐跳跃，仅能从外排封面密度倒推。')
    lines.append('4. **选题维度命名约束**：在选题维度拆解中，基于全量笔记标题进行语义归类，给出 4-6 个内容特有的维度名称。每个维度包含：维度名、笔记数量 N 篇（占比 X%）、平均点赞、代表性笔记标题、战略意图与核心痛点。禁止使用万能模板标签。')
    lines.append("```")
    lines.append("")
